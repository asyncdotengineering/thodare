# Thodare Backend Abstraction — Proposal

> **Status:** research-grade proposal, not an RFC. Author: Claude (this session, 2026-05-02). Audience: Mithushan + the next session that opens an RFC at `rfcs/backend-abstraction/`. Scope: architectural, not implementation.

**One-line summary.** Refactor Thodare's hard dependency on `@thodare/openworkflow` into a thin **Backend** port (≤10 methods + capability flags), ship 5 first-party adapters that cover serverless / managed / self-host / dev, keep the JSON+EditOp surface untouched, and inherit the runtime track records of every serious durable-execution engine in the ecosystem.
...
Today the runtime is hardcoded to `@thodare/openworkflow` (vendored Postgres-backed substrate). The next architectural move is to **swap "openworkflow" for an abstract Backend contract**, so the same workflow JSON + LLM patch loop runs against:
...
- **Vercel WDK** (free inheritance of Vercel's `backend-vercel` / `backend-postgres` / `backend-local` / community backends)
...
This is the right architectural moment to formalize the Backend port: the pattern is now industry-validated, the contract is well-understood, and Thodare is small enough that the refactor is bounded (the openworkflow coupling surface is ~5 files, ~700 LOC out of ~5k engine LOC — see `_scratch-interface-design.md`).
...
The Backend abstraction solves all three with the same change.
...
### 3.2 The `Backend` interface (informative — not a code change)
...
**Total surface: 8 methods on `Backend` + 3 on `Step` + a capability bag.** Documentable on a single page.

### 3.3 Contract test suite (the load-bearing operational discipline)

Every adapter must pass the same vitest suite (`@thodare/backend-contract-tests`), parameterized over the adapter under test. The suite covers:
...
|---|---|---|---|---|
| `@thodare/backend-openworkflow-pg` | **Default**, no behavior change for existing users | Postgres + openworkflow | trivial wrapper; ~150 LOC | "Production self-host" |
| `@thodare/backend-openworkflow-sqlite` | `thodare dev` ergonomics — single-binary local | SQLite + openworkflow | trivial wrapper; shares 90% with above | "Zero-config dev" |
| `@thodare/backend-cloudflare-dynamic` | **Serverless-managed**, lifts CF Workflows + `cloudflare/dynamic-workflows` | CF Workflows + D1 + Queues | ~600 LOC; non-trivial because of `vm.Context` absence | "Pay-per-use serverless" |
| `@thodare/backend-wdk` | **Inheritance play** — wraps Vercel's WDK so all 7 official + community Backends become Thodare backends | Whatever WDK can use (Vercel/Postgres/Local/Turso/MongoDB/Redis/Jazz) | ~400 LOC + WDK peer dep | "Pick any WDK World" |
| `@thodare/backend-inngest` | Managed-serverless via Inngest | Inngest + their queue | ~300 LOC; SDK already exposes `step.run/sleep/waitForEvent` | "I'm already on Inngest" |

**v0.2 ships seven backends total** through five packages (because `backend-wdk` lifts WDK's seven backends). That's the value proposition.

### 4.2 Ships in v0.3+ (validated by user demand, not assumption)

| Package | When | Why deferred |
|---|---|---|
| `@thodare/backend-rivetkit-engine` | A user asks for it | Rivet's `@rivetkit/workflow-engine` is a peer; pluggable but not strategic for v1 |
| `@thodare/backend-temporal` | A user with Temporal asks | Temporal's worker model is the "we don't want this" reference; only build if a real customer pays for it |
| `@thodare/backend-lambda-sqs` | A user without CF or Vercel asks | Wider adoption depends on it; deferred until v0.2 lands and signals point this way |
| `@thodare/backend-native` | Probably never | The native runtime is the "Alternative A" trap. Build only if a clear gap nothing else fills emerges. |
...
#### `@thodare/backend-openworkflow-pg` (the trivial case — current behavior)
...
#### `@thodare/backend-cloudflare-dynamic` (the interesting case)
...
**Hidden value:** because `backend-cloudflare-dynamic` uses CF Workflows directly (not a custom orchestrator on Queues+DO+D1), pricing collapses from `cloudflare-as-world.md`'s Option B (~$10.7k/mo) toward Option A (~$6.1k/mo) at 10M runs/day. **The `dynamic-workflows` library is the architectural unlock that makes the CF adapter affordable at Black Friday scale.**

#### `@thodare/backend-wdk` (the inheritance play)
...
**Capability flags:** depend on the underlying WDK World. The adapter introspects the Backend instance and forwards `backend.specVersion`, then sets `serverless: true` for `backend-vercel`, `false` for `backend-postgres`, etc.
...
**Citation note:** Inngest's published SDK Spec is the most copyable artifact in the durable-engines survey. If Thodare ever ships a wire-protocol Backend instead of an in-process one, this is the prior art.
...
- `packages/backend/` (new) — pure types + the `ThodareBackend` interface + `BackendCapabilities` + `ThodareStep` + `ThodareCtx`. No runtime code, no dependencies. Mirror `@workflow/world` from WDK in structure (it's been validated by the public).
- `packages/backend-contract-tests/` (new) — the parameterized vitest suite. Imports `@thodare/backend` types only. Provides `runContractTests(backend, options?)`.
- RFC at `rfcs/backend-abstraction/README.md` — restate this proposal in RFC form. Lock the interface and capability list in v0 contract; bump in v1+ with explicit semver discipline.

### Phase 2 — Extract the openworkflow adapter (~1 week)

- `packages/backend-openworkflow-pg/` (new) — wraps `OpenWorkflow` + `BackendPostgres`. ~150 LOC. Passes contract tests.
- `packages/backend-openworkflow-sqlite/` (new) — same code, different `Backend`. Trivial alias package.
- `packages/engine/src/runner/openworkflow.ts` and `runtime-workflow.ts` and `handle.ts` — refactor to take a `Backend` instead of an `OpenWorkflow` + `Backend` pair. Internal change; the `walk.ts` walker is already abstract (`step: any`). **Backward-compatible**: `createWfkit({ backend })` continues to work, but is documented as deprecated in favor of `createWfkit({ backend })`.
...
- `packages/backend-cloudflare-dynamic/` (new) — uses `cloudflare/dynamic-workflows@0.1.1` + `@cloudflare/workers-types` + a D1 client. ~600 LOC.
...
- `packages/backend-wdk/` (~400 LOC + WDK peer dep)
- `packages/backend-inngest/` (~300 LOC + Inngest SDK peer dep)
...
**`BuildPlugin` interface** (mirror Flue): each `backend-*` package exports a `BuildPlugin` with five methods (`generateEntryPoint`, `bundle?`, `entryFilename?`, `esbuildOptions?`, `additionalOutputs?`). The CLI `--target` flag dispatches to the matching plugin. Third parties can write their own targets without touching the CLI.
...
**Mitigation:** capability flags carry the truth; the validator at `applyOperations` time refuses to register a workflow whose declared step output exceeds the active Backend's `maxStepOutputBytes`; the contract-test suite asserts the per-adapter behavior against documented slack windows.

**The honest framing in docs:** "Thodare unifies the surface, not the substrate. Pick the Backend whose tradeoffs match your workload."

### 7.2 The "make Thodare a WDK World instead" alternative

**The temptation:** instead of wrapping WDK as one of many Backends, *be* a WDK World — implement the `@workflow/world` interface (Storage + Queue + Streamer) backed by openworkflow + Thodare's stores. Then any `"use workflow"` TS code from the WDK ecosystem runs on Thodare's substrate.
...
**The answer:** because none of them ship the JSON+EditOp+multi-tenant-API+Diataxis-docs-discipline+`hidden()`-secret-boundary surface. Thodare is the **LLM-native control plane on top of any of them**. The Backend abstraction makes that pitch credible — adopters can use the substrate they already trust.

This is the strategic positioning the Backend abstraction unlocks.
...
- [ ] Migration codemod from `{ backend }` → `{ backend }` ships in the same release.
...
- [ ] **10,000 concurrent workflow runs** sustained for 1 hour against `backend-openworkflow-pg`, with p99 step latency ≤ 200ms and zero data loss on a single worker pod restart mid-run.
- [ ] **1,000 concurrent runs** sustained for 1 hour against `backend-cloudflare-dynamic`, with p99 step latency ≤ 1s and zero replay-divergence errors.
...
- [ ] At least one external adopter writes a third-party Backend adapter using the public `@thodare/backend` types within 90 days of v0.2 release. (If nobody can write a third-party adapter, the abstraction is wrong.)
...
1. **Capability flags vs. trait composition.** This proposal recommends capability flags (Alternative B). If the maintainer prefers traits (Alternative C), the Backend interface and the contract-test suite need to be rebuilt around traits — meaningful redesign, not a tweak.
...
3. **License posture for community adapters.** Vendored CF `dynamic-workflows` is MIT (compatible). WDK is Apache-2.0 (already vendored as `@thodare/openworkflow`). Inngest SDK is Apache-2.0. Rivet is Apache-2.0. The default policy ("MIT for our work, Apache-2.0 for vendored, document in NOTICE per T19") covers everything. **No new license decisions needed** unless a community Backend ships under SSPL or BSL.
...
**End of proposal.** Open the RFC at `rfcs/backend-abstraction/README.md` when ready.
